...
from my_randomgazebomaster_pkg.srv import ArmMove, ArmMoveResponse, ArmMoveRequest
import copy
from my_randomgazebomaster_pkg.rvzi_markers import MarkerBasicsArray
from my_randomgazebomaster_pkg.rvzi_markers import MarkerBasics
import yaml
import rospkg
import random
...
class MoveArmClient(object):
    ...
    def __init__(self):
        ...
        rospy.wait_for_service('/robot1/arm_controller/follow_joint_trajectory')
        
        self.robot = moveit_commander.RobotCommander()
        self.scene = moveit_commander.planning_scene_interface()
    ...
    def move_endeffector(self, position_XYZ, orientation_XYZW):
        ...
        pose_target.position.x = position_XYZ[0]
        pose_target.position.y = position_XYZ[1]
        pose_target.position.z = position_XYZ[2]
        
        pose_target.orientation.x = orientation_XYZW[0]
        pose_target.orientation.y = orientation_XYZW[1]
        pose_target.orientation.z = orientation_XYZW[2]
        pose_target.orientation.w = orientation_XYZW[3]
    ...
    def move_joints(self, joints_config):
        ...
        pose_target = geometry_msgs.msg.Pose()
        pose_target.position.x = joints_config[0]
        pose_target.position.y = joints_config[1]
        pose_target.position.z = joints_config[2]
        pose_target.orientation.x = joints_config[3]
        pose_target.orientation.y = joints_config[4]
        pose_target.orientation.z = joints_config[5]
        pose_target.orientation.w = joints_config[6]
    ...
    def move_gripper(self,gripper_x, max_effort):
        ...
        pose_requested = Pose()
        self.arm_move_request.pose = pose_requested
        
        self.arm_move_request.joints_array = [gripper_x,max_effort]
        
        self.arm_move_request.movement_type.data = "GRIPPER"
        
        result =  self.arm_move_service(self.arm_move_request)
...
def testing_positions():
    ...
    Arm_move_client = MoveArmClient()
    
    position_XYZ = [0.598,0.005,0.9]
    orientation_XYZW = [0, 0, 0, 1]
    
    Arm_move_client.move_endeffector(position_XYZ, orientation_XYZW)
    ...
    #Arm_move_client.move_joints(arm_joint_positions)
...
def test_range_world_movements():
    """
    Here we test the range above the table that Arm can move the arm.
    """
    marker_size = 0.1
    marker_array_obj = MarkerBasicsArray(namespace="Arm_test_range", marker_size=marker_size)
    ...
    while position_XYZ[0] <= max_value:
        ...
        while position_XYZ[1] <= max_value:
            ...
            result = Arm_move_client.move_endeffector(position_XYZ, orientation_XYZW)
    ...
    pose_files_dir = os.path.join(path_to_package, "Arm_limits_files")
    
    if not os.path.exists(pose_files_dir):
        os.makedirs(pose_files_dir)
    
    pose_file_name = "Arm_move_limits.yaml"
...
def test_grasp():
    ...
    gripper_x = 0.1
    max_effort = 10.0
    Arm_move_client.move_gripper(gripper_x, max_effort)
    ...
    height_delta = 0.19
    position_XYZ = [0.0, 0.0, 0.6449038 ]
    orientation_XYZW = [-0.707, 0.000, 0.707, 0.001]
...
if __name__ == '__main__':
    rospy.init_node('Arm_move_client_node', anonymous=True, log_level=rospy.INFO)
