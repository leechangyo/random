...
from my_randomgazebomaster_pkg.move_arm_client import MoveArmClient
...
def dataset_generator(models_of_interest_list, models_to_be_list, full_path_to_dataset_gen='./dataset_gen/', number_of_dataset_elements=10, frequency=1, show_images=False, env_change_settle_time=2.0, move_arm=False, move_time=2.0, index_img=0):
    ...
    for i in range(number_of_dataset_elements):
        ...
        if move_arm:
            ...
            move_arm_client_object.move_tcp_to_random_pose(x_range,y_range,z_range,orientation_XYZW)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 10:
        print("usage: create_training_material.py models_of_interest_list models_to_be_list number_of_elements env_change_settle_time show_images move_arm move_time init_index_img path_to_dataset_gen")
    else:
        ...
        if path_to_dataset_gen == "None":
            path_to_dataset_gen = rospack.get_path('my_randomgazebomaster_pkg')
