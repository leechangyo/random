...
public:
      ...
      for (auto link : model->GetLinks()) {
        ...
        if (linkSDF->HasElement("visual")) {
          for (sdf::ElementPtr visualSDF = linkSDF->GetElement("visual");
            ...
            gazebo::common::Color ambient = materialSDF->GetElement("ambient")
                                                ->Get<gazebo::common::Color>();
            gazebo::common::Color diffuse = materialSDF->GetElement("diffuse")
                                                ->Get<gazebo::common::Color>();
    ...
    for (auto model : this->world->GetModels()) {
      ...
      if (!this->check_if_vector_contains_string(this->model_to_ignore_movement,
        ...
        gazebo::math::Pose initPose(
            math::Vector3(x_pos_rand, y_pos_rand, z_pos_rand),
            math::Quaternion(roll_rand, pitch_rand, yaw_rand));
  ...
  bool ChangeLight(std::string light_name) {
    ...
    if (this->colour_randomiser_active == "yes") {
      gazebo::msgs::Set(
          msg.mutable_diffuse(),
          common::Color(diffuse_R, diffuse_G, diffuse_B, diffuse_A));
      gazebo::msgs::Set(
          msg.mutable_specular(),
          common::Color(specular_R, specular_G, specular_B, specular_A));
    } else {
  ...
  float UnNormaliseFloat(float x_normalised, float x_norm_min,
                         float x_norm_max) {
    float x_unnormalised;
    ROS_DEBUG("x_normalised=[%f]", x_normalised);
    ROS_DEBUG("x_norm_min x_norm_max=[%f,%f]", x_norm_min, x_norm_max);
    x_unnormalised = (x_normalised - (x_norm_min / (x_norm_max - x_norm_min))) *
                     (x_norm_max - x_norm_min);
  ...
  float RandomFloatBeta(float a, float b) {
    ...
    // We have to consider three cases: a and b pos, a and b negative, a
    // negative b positive
    int case_id;
    float delta;
    float ax;
    ...
    ROS_DEBUG("A B [%f,%f]", a, b);
    if (a > 0.0f && b > 0.0f) {
      // We dont change anything
      ROS_DEBUG("a > 0.0f && b > 0.0f");
      ax = abs(a);
      bx = abs(b);
      case_id = 0;
      ROS_DEBUG("IN AX Bx=[%f,%f]", ax, bx);
      ...
      ROS_ERROR("The Y Ranges are wrong=[%f,%f]", a, b);
      ax = 0.0f;
      bx = 0.0f;
      ROS_DEBUG("IN AX Bx=[%f,%f]", ax, bx);
    }
    ...
    // quantile needs a positiove RandomFloat. Thats why the if clauses
    // before.
    float ax_norm = 0.0;
    float bx_norm = 1.0;
    ROS_DEBUG("AX Bx=[%f,%f]", ax, bx);
    ROS_DEBUG("AX-NORM Bx-NORM=[%f,%f]", ax_norm, bx_norm);
    float randFromUnif = RandomFloat(ax_norm, bx_norm);
    ROS_DEBUG("randFromUnif=[%f]", randFromUnif);
    ...
    ROS_DEBUG("BETA RAND VALUE=%f....", randFromDist);

    // Now we unnormalise that value into the ax and bx limits
    float randFromDist_unnorm = UnNormaliseFloat(randFromDist, ax, bx);
    ROS_DEBUG("BETA RAND VALUE UNORM=%f....", randFromDist_unnorm);

    return randFromDist_unnorm;
  }
...
private:
  std::map<int, gazebo::common::Color> lastKnownColors;
